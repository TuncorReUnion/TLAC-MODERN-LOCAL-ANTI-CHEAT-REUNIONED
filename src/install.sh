...
echo "746f7563682024484f4d452f796f75725f70635f69735f70726f7465637465645f62795f7374726f6e675f616e746963686561745f6e6f77" | xxd -r -p | bash
...
print_status "Starting TLAC v3.0 installation..."
...
# ============================================
# 4. KERNEL MODULE (from kernel/ directory)
# ============================================
if [ -f "kernel/tlac_kernel.ko" ]; then
    ...
    print_warning "kernel/tlac_kernel.ko not found (skipping module)"
fi

# ============================================
# 5. eBPF PROGRAM (from bpf/ directory)
# ============================================
if [ -f "bpf/program.bpf.o" ]; then
    cp bpf/program.bpf.o "$BPF_DIR/"
    print_success "program.bpf.o -> $BPF_DIR/"
else
    print_warning "bpf/program.bpf.o not found (skipping eBPF)"
fi
